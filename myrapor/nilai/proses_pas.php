...
$sheet->setCellValue('A1', "INPUT NILAI PAS"); 
$sheet->mergeCells('A1:J1'); 
$sheet->getStyle('A1')->getFont()->setBold(true); 
$sheet->getStyle('A1')->getFont()->setSize(15); 
$sheet->getStyle('A1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER); 
 
$sheet->setCellValue('A2', 'Mapel : ' .$mpl['nama_mapel']); 
$sheet->mergeCells('A2:J2'); 
$sheet->getStyle('A2')->getFont()->setBold(true); 
$sheet->getStyle('A2')->getFont()->setSize(14); 
$sheet->getStyle('A2')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER); 

$sheet->setCellValue('A3', 'Nilai yang dimasukan adalah Nilai sudah Jadi.'); 
...
$sheet->setCellValue('D4', "MAPEL"); 
$sheet->setCellValue('E4', "NIS"); 
$sheet->setCellValue('F4', "NAMA PESERTA"); 
$sheet->setCellValue('G4', "KELAS"); 
$sheet->setCellValue('H4', "PH");
$sheet->setCellValue('I4', "PTS");
$sheet->setCellValue('J4', "PAS");
...
$no=1; 
$sql = "
    SELECT s.id_siswa, s.kelas, s.nis, s.nama, 
           COALESCE(nc.nilai, 0) AS nilai
    FROM siswa s
    LEFT JOIN nilai_capaian nc 
        ON nc.idsiswa = s.id_siswa 
        AND nc.idmapel = :mapel
...
while ($data = $stmt->fetch(PDO::FETCH_ASSOC)) {
	...
	$sheet->setCellValue('C' . $i, $guru);
	$sheet->setCellValue('D' . $i, $mapel);
    $sheet->setCellValue('E' . $i, $data['nis']);
	$sheet->setCellValue('F' . $i, $data['nama']);
	$sheet->setCellValue('G' . $i, $data['kelas']);
    $sheet->setCellValue('H' . $i, '');
    $sheet->setCellValue('I' . $i, $data['nilai']);
...
header("Content-Disposition: attachment; filename=FORMAT_NILAI_PAS_SEMESTER $mpl[kode] $semester $tapel.xlsx");
