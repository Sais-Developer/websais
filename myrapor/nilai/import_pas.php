<?php
require "../../konek/koneksi.php"; // $pdo harus instance PDO
    ...
    if (in_array($extension, $ext)) {
        ...
        for ($a = 7; $a < 10; $a++) {
            ...
            for ($i = 4; $i < count($sheetData); $i++) {
                ...
                $stmt = $pdo->prepare("
                    SELECT COUNT(*) FROM nilai_rapor 
                    WHERE idsiswa=:ids AND idmapel=:idm AND kodenilai=:kode AND ket=:ket 
                ...
                if ($cek == 0) {
                    ...
                    $stmt = $pdo->prepare("
                        ...
                        VALUES (:ids, :nis, :idm, :kelas, :nilai, :kode, 'PAS', :semester, :tapel, :guru)
                    ");
                    $stmt->execute([
                        ':ids' => $ids,
						':nis' => $nis,
