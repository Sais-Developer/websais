			...
			<div class="card-body">
					<table id="datatable1" class="table table-bordered" style="width:100%;font-size:12px">
						...
						$stmt = $pdo->prepare("
							...
							WHERE s.kelas = ? AND nr.idmapel = ? AND nr.ket = ? AND nr.semester = ? AND nr.tapel = ?
						");
						$stmt->execute([$kelasmu, $mapelmu, $ket, $semester, $tapel]);
						$siswaData = [];
						...
						<tr>
							...
							<td>
								<?php if ($jumlah == 0): ?>
									<a href="?pg=<?= enkripsi('pts') ?>&ids=<?= $data['ids'] ?>&g=<?= $gurumu; ?>&k=<?= $kelasmu; ?>&m=<?= $mapelmu; ?>&nr=<?= $nr ?>">
			...
			<div class="col-md-12 mb-1">
				...
				<div class="col-md-12 mb-1">
				   <label class="bold">Guru</label>
						<select name="guru" id="guru" class="form-select" style="width:100%" required>
					...
					<div class="widget-payment-request-actions m-t-lg d-flex">
					 <button id="pilih" class="btn btn-primary flex-grow-1 m-l-xxs">Pilih Kelas</button>
						...
						<div class="d-flex justify-content-between align-items-center mb-2">
							...
							<a href="nilai/proses_pts.php?k=<?= urlencode($kelasmu); ?>&m=<?= urlencode($mapelmu); ?>&g=<?= urlencode($gurumu); ?>" 
							   class="btn btn-link" 
							...
							<?php if($ids !=''): ?>
							 <div class="widget-payment-request-info m-t-md">
								...
								<?php
									...
									?>
								<tr>
								<td><input type="radio" name="tinggi" value="<?= $data['capaian'] ?>" required
...
<script type="text/javascript">
	...
	location.replace("?pg=<?= enkripsi('pts') ?>&k=" + k + "&g=" + g + "&m=" + m + "&kt=" + kt);
	}); 
</script>
<script>
