...
<head>
<meta charset='utf-8'>
<title>Rapor PAT <?= ucwords(strtolower($siswa['nama'])) ?></title>
     <style>
        @page { 
		margin-top: 1cm; 
		margin-right: 1cm; 
...
<body>
   ...
   <table class="header-table" >       
	...
	$stmtAbsen = $pdo->prepare("
			...
			FROM absen_rapor 
			WHERE nis = :nis AND semester = :semester AND tapel = :tapel AND ket = 'PAT'
		");
		$stmtAbsen->execute([
			':nis' => $nis,
			':semester' => $semester,
...
<table style="width:46%">
         <tr>
		 <td>Keputusan :
		 <br>Berdasarkan pencapaian seluruh kompetensi, peseta didik dinyatakan :
		 <br>Naik / Tinggal *) kelas .......... (...........................................)
...
<?php
		$stmt6 = $pdo->prepare("
			SELECT tanggal 
			FROM tanggal_rapor
			WHERE semester = :semester AND tapel = :tapel AND ket = 'PAT'
		");
		$stmt6->execute([
			':semester' => $semester,
			':tapel' => $tapel
    ...
    <tr>
        ...
        <td width="33%">
            ...
            Kepala Sekolah
            <br><br><br><br>
            <u><strong>(<?= $_POST['kepsek'] ?>)</strong></u><br>
            NIP. <?= $_POST['nipkepsek'] ?>
        </td>
        <td width="33%">
           <?= ucwords(strtolower($setting['kabupaten'])); ?>, <?= $rapor['tanggal'] ?><br/>
			Wali Kelas <?= $kelas ?><br>
            <br><br><br><br>
            <u><strong>(<?= $_POST['walas'] ?>)</strong></u><br>
            NIP. <?= $_POST['nipwalas'] ?>
        </td>
    </tr>
</table>
...
$dompdf->render();
$dompdf->stream("Rapor_PAT_".$siswa['nama'].".pdf", ["Attachment" => false]);
exit(0);
?>
