...
$fase = fetch('m_kelas',['kelas' => $kelas]);

$stmtSiswa = $pdo->prepare("SELECT * FROM siswa WHERE nis = :nis LIMIT 1");
...
<head>
<meta charset='utf-8'>
<title>Rapor PTS <?= ucwords(strtolower($siswa['nama'])) ?></title>
     <style>
        @page { 
		margin-top: 1cm; 
		margin-right: 1cm; 
...
<body>
    ...
    <h4 class="text-center">LAPORAN HASIL BELAJAR<br>ASESMEN SUMATIF TENGAH SEMESTER</h4>
    <br>
   <table class="header-table" >       
      ...
      <table>
		...
		<table style="width:100%">
		 ...
		 <?= ucwords(strtolower($siswa['nama'])) ?> mampu <?= $koku['mampu'] ?>, namun perlu bimbimgan dalam <?= $koku['kurang'] ?></td>
		 <?php endif; ?>
		 </td>
		 </tr>
		 </table>
	...
	<?php
		$stmtAbsen = $pdo->prepare("
			...
			FROM absen_rapor 
			WHERE nis = :nis AND semester = :semester AND tapel = :tapel AND ket = 'PTS'
		");
		$stmtAbsen->execute([
			':nis' => $nis,
			':semester' => $semester,
    ...
    <td style="width:45%; vertical-align:top; border:none;">
      <table class="inner-table" style="margin-left:5px">
		...
		$stmt6 = $pdo->prepare("
			SELECT tanggal 
			FROM tanggal_rapor
			WHERE semester = :semester AND tapel = :tapel AND ket = 'PTS'
		");
		$stmt6->execute([
			':semester' => $semester,
			':tapel' => $tapel
...
$dompdf->render();
$dompdf->stream("Rapor_PTS_".$siswa['nama'].".pdf", ["Attachment" => false]);
exit(0);
?>
