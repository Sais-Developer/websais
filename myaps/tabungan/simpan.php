...
try {
    $stmt = $pdo->prepare("INSERT INTO saldo (tanggal, jam, idsiswa, debet) VALUES (:tanggal, :jam, :idsiswa, :debet)");
    $stmt->bindParam(':tanggal', $tanggal, PDO::PARAM_STR);
    $stmt->bindParam(':jam', $jam, PDO::PARAM_STR);
    $stmt->bindParam(':idsiswa', $idsiswa, PDO::PARAM_INT);
    $stmt->bindParam(':debet', $duit, PDO::PARAM_INT);
    $stmt->execute();

    $stmt = $pdo->prepare("UPDATE siswa SET saldo = saldo + :debet WHERE id_siswa = :idsiswa");
    $stmt->bindParam(':debet', $duit, PDO::PARAM_INT);
    $stmt->bindParam(':idsiswa', $idsiswa, PDO::PARAM_INT);
    $stmt->execute();
    $pdo->commit();

    $stmtSis = $pdo->prepare("SELECT nowa, nama, saldo FROM siswa WHERE id_siswa = :idsiswa LIMIT 1");
    $stmtSis->bindParam(':idsiswa', $idsiswa, PDO::PARAM_INT);
    $stmtSis->execute();
    $siswa = $stmtSis->fetch(PDO::FETCH_ASSOC);

    $nowa = $siswa['nowa'];
    $notif = "Assalamualaikum wr.wb\n\nKami informasikan kepada Orang Tua ananda *".$siswa['nama']. "*\nBahwa hari ini tanggal *".date('d M Y')."* telah menabung sebesar Rp *".number_format($duit, 0, ',', '.')."* dengan *Total Saldo* saat ini sebesar *Rp ".number_format($siswa['saldo'], 0, ',', '.')."*\n\nDemikian Informasi ini disampaikan secara Otomatis oleh *Server ".$setting['sekolah']."*\nHarap menjadi Monitoring Tabungan siswa, kepada Orang Tua Siswa. Tidak perlu dibalas. Terima kasih\n\nWasalamualaikum wr.wb";
