...
<hr style="margin:2px">
  ...
  <h4 class="text-center">DAFTAR PENARIKAN TABUNGAN<br> BULAN <?= strtoupper($bulane['ket']) ?> <?= $tahun ?></h4>
		<br>
				 <table width='100%'>       
					  <tr>
						<th>NO</th>                                               
			   ...
			   <?php
				...
				$sql = "SELECT t.tanggal, MONTH(t.tanggal) AS bulan, t.kredit, s.nama
						FROM saldo t
						LEFT JOIN siswa s ON s.id_siswa = t.idsiswa
						WHERE t.kredit > :zero AND MONTH(t.tanggal) = :bulanmu";
				...
				?>
					<tr>
						<td colspan="3" style="text-align:right"><strong>Total Penarikan:</strong></td>
						<td style="text-align:right"><strong><?= number_format($totalKredit, 0, ',', '.'); ?></strong></td>
					</tr>
				<?php
...
<table class="header-table" width='100%'>
	...
	NIP. <?= $setting['nip'] ?>
		...
		<td width="30%" style="border: 1px solid #000; padding: 10px; text-align: center; vertical-align: top;">
			...
			<br/>
			<i><?= ucfirst(terbilang($totalKredit)) . " rupiah"; ?></i>
			<br/><br/><br/>							
		</td>
	   <td width="5%"></td>						
		<td>
...
$dompdf->render();
$dompdf->stream("Rekap Penarikan Bulan ". $bulane['ket'] . ".pdf", array("Attachment" => false));
exit(0);
?>