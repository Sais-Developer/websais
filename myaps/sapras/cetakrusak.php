...
require("../../konek/crud.php");
	...
	$sql_rusak = "SELECT s.*, r.nama_ruangan, k.kategori
	FROM sapras s 
	LEFT JOIN sapras_kate k ON k.id=s.idk 
	LEFT JOIN sapras_ruangan r ON r.id=s.lokasi_id
	WHERE s.kondisi IN ('Rusak Ringan', 'Rusak Berat') ORDER BY s.nama_barang";
	$stmt_rusak = $pdo->prepare($sql_rusak);
	$stmt_rusak->execute();
	?>
...
<hr style="margin:2px">
  ...
  <h4 class="text-center">REKAPITULASI BARANG DALAM KONDISI RUSAK<br> BULAN <?= strtoupper($bulane['ket']) ?> <?= $tahun ?></h4>
		<br>
		 <table width='100%'>       
			  <tr>
				 <th>No.</th>
			   ...
			   <?php
            $no = 1;
            while ($data_rusak = $stmt_rusak->fetch(PDO::FETCH_ASSOC)) {
                echo "<tr>";
                echo "<td class='text-center'>" . $no++ . "</td>";
                echo "<td>" . $data_rusak['nama_barang'] . "</td>";
                echo "<td>" . $data_rusak['kategori'] . "</td>"; 
                echo "<td class='text-center'>" . $data_rusak['jumlah'] . "</td>";
                echo "<td>" . $data_rusak['nama_ruangan'] . "</td>"; 
                echo "<td>" . $data_rusak['keterangan'] . "</td>";
                echo "</tr>";
            }
            ?>
				</table>
...
$dompdf->render();
$dompdf->stream("Rekap_Kondisi_Rusak Bulan ". $bulane['ket'] . ".pdf", array("Attachment" => false));
exit(0);
?>