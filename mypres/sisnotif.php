...
function getPesan($db, $id) {
    $stmt = $db->prepare("SELECT * FROM m_pesan WHERE id = :id");
    $stmt->execute(['id' => $id]);
    $data = $stmt->fetch(PDO::FETCH_ASSOC);
    return $data;
}

$pesan5 = getPesan($db, 5);
$pesan6 = getPesan($db, 6);
?>
...
{
	type: 'POST',
	 url: 'tsetting.php?pg=masukeksis',
	data: data,
	cache: false,
	contentType: false,
	processData: false,
...
{
	type: 'POST',
	 url: 'tsetting.php?pg=pulangeksis',
	data: data,
	cache: false,
	contentType: false,
	processData: false,
