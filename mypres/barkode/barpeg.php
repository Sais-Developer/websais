	...
	<div class="text-muted"><?= $setting['sekolah'] ?></div>
	   ...
	   <div class="widget-payment-request-info m-t-md">
		   <div class="d-grid gap-2 mb-4">
			  <a href="?pg=<?= enkripsi('barpeg') ?>&ac=<?= enkripsi('pegawai') ?>" 
				...
				<tbody>
					<?php
						...
						<tr>
							...
							<td>
								<a href="?pg=<?= enkripsi('barpeg') ?>&ac=<?= enkripsi('pegawai') ?>&idp=<?= intval($data['id_guru']) ?>"
...
$('#formkartu').submit(function(e){
	...
	$.ajax(
	{
		...
		setTimeout(function()
			{
			window.location.replace('?pg=<?= enkripsi("barpeg") ?>&ac=<?= enkripsi("pegawai") ?>');
		}, 200);
